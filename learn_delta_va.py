...
class PromptCombinationDataset_VA(Dataset):
    def __init__(self, prompts: Dict[str, str], prefixes: List[str]) -> None:
        super().__init__()
        self.prompts = prompts
        self.prefixes = prefixes
    ...
    def __getitem__(self, index) -> Dict[str, str]:
        i_prompt = index // len(self.prefixes)
        prefix = self.prefixes[index - i_prompt * len(self.prefixes)]
        return { k: ([f'{prefix}', f'{v}'] if 'prompt' in k else v) for k, v in self.prompts[i_prompt].items() }
    ...
    for global_step in (pbar := tqdm(range(max_steps), desc='Optimizing')):
        ...
        for accum_step in range(grad_accum_steps):
            with torch.no_grad():
                ...
                prompt_target, tokenwise_direction = model.embed_prompt_va(batch['prompt_target'][0][0], batch['prompt_target'][1][0])
                prompt_positive, prompt_negative = copy.deepcopy(prompt_target), copy.deepcopy(prompt_target)
                prompt_positive.tokenwise_embeddings['oai_clip_vit_l'] += tokenwise_direction[...,:768]
                prompt_negative.tokenwise_embeddings['oai_clip_vit_l'] -= tokenwise_direction[...,:768]
                prompt_positive.tokenwise_embeddings['openclip_g'] += tokenwise_direction[...,768:]
                prompt_negative.tokenwise_embeddings['openclip_g'] -= tokenwise_direction[...,768:]
                prompts_embedded = { 
                    'prompt_target': [prompt_target,],
                    'prompt_positive': [prompt_positive,],
                    'prompt_negative': [prompt_negative,],
                    'pattern_target': batch['pattern_target'][0]
                } # Each key has tensors of size: [77, 768], [77, 1280], [768], [512]
